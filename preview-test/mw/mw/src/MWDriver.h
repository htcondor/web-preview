...
private:
  ...
  void addTaskByKey( MWTask *add_task , MWTask *& hand, MWTask *& handprev );
...
public:
  ...
  /** Sets the desired number of workers.  This takes an array of 
      ...
      each arch class.  */
  void set_target_num_workers( int iWantThisMany[], int num_arches );
...
public:	
  ...
  /** Another terrible addition so that Jeff can print out the worker list
      in his own format */
  MWWorkerID *get_workers_head() { return workers; }
