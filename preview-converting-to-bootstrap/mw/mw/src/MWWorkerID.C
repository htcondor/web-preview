...
int *MWWorkerID::vids = new int[MW_MAX_WORKERS];  /* 1024 workers enough for now! */
...
void MWWorkerID::get_machine_info() {
	...
	if( jeff_hack ) {
	  sprintf( temp_str, "./create_condor_status_file %s %s", mach_name, filename );
	}
	else {
	  
	  memset(temp_str, '\0', sizeof(temp_str));
	  sprintf(temp_str, "/unsup/condor/bin/condor_status -l %s > %s", 
		  mach_name, filename);
	}
	...
	if ((inputfile = fopen(filename, "r")) == 0) {
		MWprintf( 10, "Failed to open condor_status file, errno %d\n", errno);
		return;
    } else {
...
void
MWWorkerID::setNetworkConnectivity ( MWTask *nTask )
{
	struct timeval timeVal;
	gettimeofday ( &timeVal, NULL );
	lastMeasuredTime = timeval_to_double ( timeVal );
	networkConnectivity = nTask;
}

MWTask*
MWWorkerID::getNetworkConnectivity ( double &retVal )
{
	retVal = lastMeasuredTime;
