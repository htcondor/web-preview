...
/// How many more hosts to ask for each time
const int HOSTINC = 6; 
...
MWFileRC::WaitOnExistFile ( )
{
    ...
    while ( 1 )
    {
        if ( worker_time + worker_timeout < time(0) )
	{
		MWprintf ( 10, "Dying since no response\n");
		exit(0);
	}
...
MWFileRC::init_beginning_workers ( int *nworkers, MWWorkerID ***workers )
{
    ...
    *nworkers = j;
	...
	if( target_num_workers > REAL_TARGET_NUM_WORKERS ) {
	  MWprintf( 10, "target_num_workers too big.  Setting to %d\n",  REAL_TARGET_NUM_WORKERS );
	  target_num_workers = REAL_TARGET_NUM_WORKERS;
	}
...
MWFileRC::do_spawn ( int nWorkers, int archid )
{
  ...
  if ( (ptr = popen(exe, "r") ) != NULL ) {
    ...
    for ( i = 0; i < nWorkers; i++ ) {
      ...
      fileWorkers[tempp].num_starts = 1;
      for ( int j = 0; j < 2 * HOSTINC; j++ ) {
...
MWFileRC::setup_notifies ( int worker_tid )
{
    ...
    workerEvents[worker_tid].tag[FileChecksumError] = CHECKSUM_ERROR;
...
MWFileRC::handle_finished_worker ( int i )
{
	...
	msgTag = workerEvents[i].tag[FileChecksumError];
    }
...
MWFileRC::handle_transited_worker ( int i )
{
    ...
    current_num_workers--;
     ...
     hostadd_reqs[fileWorkers[i].arch]++;
    //XXX HOSTADDDEBUG
    MWprintf( 20, "Adding to %d hostadd_reqs\n", fileWorkers[i].arch,
	      hostadd_reqs[fileWorkers[i].arch] );
    ...
    // Copying log file...
     ...
     //char jtcommand[1024];
     //sprintf( jtcommand, "cp -f submit_files/log_file.%d /tmp/linderot", whomRecv );
     //system( jtcommand );
    // end jeff's debugging
...
MWFileRC::handle_transited_worker ( int i )
{
    ...
    // Jeff's debugging!!!
    //XXX NOT ADDING to hostadd_reqs for the time being...
    hostadd_reqs[fileWorkers[i].arch]++;
    MWprintf( 20, "Adding to hostaddreqs for arch %d.  Value %d\n", fileWorkers[i].arch,
	      hostadd_reqs[fileWorkers[i].arch] );
...
MWFileRC::CheckLogFilesRunning ( )
{
	...
	for ( ;  outcome == ULOG_OK ;outcome = log.readEvent( event ) )
	{
	    ...
	    else if ( event->cluster != fileWorkers[i].condorID || event->proc != fileWorkers[i].condorprocID )
	    {
		...
		{      
		  ...
		  if ( evictEvent->checkpointed == TRUE ) 
		    ...
		    {
		      char exe[_POSIX_PATH_MAX];
		      MWprintf ( 10, "Got an evict with no checkpoint event from %d\n", i ); 
		      sprintf ( exe, "condor_rm %d.%d", fileWorkers[i].condorID,  
				fileWorkers[i].condorprocID); 
		      system ( exe );
		      MWprintf( 10, "Executing %s\n", exe );
		      fileWorkers[i].state = FILE_KILLED;
		      fileWorkers[i].event_no++;
		    }
		}
		...
		{
		  // More of Jeff's debugging.  Why isn't this the same as FILE_KILLED about.
		  //  trying to implement this here instead of calling
		  //  FILE_TRANSIT...
...
MWFileRC::hostaddlogic ( int *num_workers )
{
    ...
    //XXX HOSTADDDEBUG
    MWprintf( 20, "Adding  %d to %d hostadd_reqs.  Value %d\n", howmany[i], i,
	       hostadd_reqs[i] );
...
MWFileRC::InitStructures ( )
{
    ...
    for ( i = 0; i < max_num_workers; i++ )
    {
    	...
    	newEvents[i].tag[FileChecksumError] = CHECKSUM_ERROR;
    }
    delete []workerEvents;
    workerEvents = newEvents;
