...
class MWRMComm {
 ...
 public:
	...
	virtual ~MWRMComm() {};
		...
		*/
	virtual void exit( int exitval ) { ::exit ( exitval ); };
	...
	virtual int read_RMstate( FILE *fp ) { return 0;};

	virtual int write_RMstate ( FILE *fp ) { return 0;};
		//@}
	...
	int get_num_arches() { return num_arches; };

		/** Set the number of arch classes */
	void set_num_arches( int n ) { num_arches = n; };
